...
class NewsUnimarController extends Controller
{
    ...
    public function index(){
        ...
        $news = News::select('news.id', 'news.title', 'news.resume', 'news.image', 'news.created_at','tags.name as first_tag')
                    ->orderBy('news.id', 'DESC')
                    ->where('news.status_id',1)
                    ->join('tags', 'news.tags_id', '=', 'tags.id');
        return view('newsunimar')->with('news', $news->paginate(7))->with('events', $this->lastEvents());
    }

    public function shownew($category, $title, $id){
        //Get the data from the selected new
        $new = News::select('news.id', 'news.title', 'news.content', 'news.image', 'news.created_at', 'news.tags_id','tags.name as first_tag')
                   ->where('news.id','=',$id)
                   ->join('tags', 'news.tags_id', '=', 'tags.id')
                   ->orderBy('id', 'DESC')
                   ->get();
        ...
        foreach(explode(",", $new[0]->tags_id) as $tag_id){
            ...
            if(count(array_unique($relatedNews, SORT_REGULAR)) < 2){
                foreach(
                News::select('news.id','news.created_at','news.title', 'news.image', 'tags.name as first_tag')
