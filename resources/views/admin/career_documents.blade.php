...
@section ('admincontent')
    ...
    <div class="d-sm-flex align-items-center justify-content-between shadow-4 mb-2">
        <h1 class="h3 mb-0 text-gray-dark">Pénsum y Lineamientos</h1>
            ...
            @foreach($documents as $document)
                <tr>
                    @if (Auth::guest())
                        ...
                        <td title="{{($document->career_name)}}">{{($document->career_name)}}</td>
                    ...
                    @else
                        ...
                        <td>
                            <button type="button" class="btn btn-warning open-modal btnedit" data-open="DocumentUpdModal"
                                    ...
                                    data-career_id="{{ $document->career_id }}"
    ...
    <div class="modal fade" id="DocumentAddModal" tabindex="-1" role="dialog" aria-labelledby="DocumentAddModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="/admin/career_documents" method="POST" enctype="multipart/form-data">
                    @csrf
                            ...
                            <div class="col-md-6">
                                <select class="form-control" id="doc_type" name="doc_type">
                                    @foreach($doc_types as $doc_type)
                                        @if($doc_type->id == '5' || $doc_type->id == '6')
    ...
    <div class="modal fade" id="DocumentUpdModal" tabindex="-1" role="dialog" aria-labelledby="DocumentUpdModal"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ModalUpdDoc">Actualizar Documentos</h5>
                    <button type="button" class="close" data-dismiss="modal" data-toggle="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{URL('/admin/career_documents/update')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <label for="careeredit" class="col-md-4 col-form-label text-md-right">Carrera: </label>
                            <div class="col-md-6">
                                <select class="form-control" id="careeredit" name="career">
                                    @foreach($careers as $career)
                                        <option value="{{$career->id}}">{{$career->name}}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            <label for="doctypeedit" class="col-md-4 col-form-label text-md-right">Tipo de documento: </label>
                            <div class="col-md-6">
                                <select class="form-control" id="doctypeedit" name="doctypeedit">
                                    @foreach($doc_types as $doc_type)
                                        @if($doc_type->id == '5' || $doc_type->id == '6')
                                            <option value="{{$doc_type->id}}">{{$doc_type->name}}</option>
                                        @endif
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            openBtn("documentos");
            activeBtn("pensum-y-lineamientos");
            $('.btnedit').click(function () {
                $('#idedit').val($(this).attr('data-id'));
                $('#doctypeedit').val($(this).attr('data-type_id'));
                $('#nameedit').val($(this).attr('data-name'));
                $('#careeredit').val($(this).attr('data-career_id'));
